...
2. Go to the lab portal web page and then go to **Settings**. Update the **webex_bot_token** variable value.Or :  you can Edit the **config.py** script. **Warning ! ! Don't forget to restart flask !**
3. Create an Alert Webex Room ([ Instructions Here ](https://github.com/pcardotatgit/Create_a_Webex_Team_Bot) ) and check that you can send messages into it from SecureX workflows. You can run the **u1_test_webex_room.py** script in the **code** folder to test your Webex Team Setup. Run it and you are supposed to receive a message into the Webex Room. If everything is Okay then Copy the webex team Room ID. 
