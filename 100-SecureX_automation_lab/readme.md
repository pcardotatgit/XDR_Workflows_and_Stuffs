...
2. Create Threat Response API client with all scopes.Go the **Administration** then Select **API Clients** in the left panel and click on the **Generate API Client** button. Click on **Select All** link in the **Scopes** Section and click on the **Add New Client** button.  Copy Threat Response **client ID** and **Client Password** and save them somewhere.
3. Install the Lab Simulator into your laptop . [see Instructions here](https://github.com/pcardotatgit/lab_simulator-001). **Start the lab Simulator**
4. Open the **config.py** script located into the simulator root directory. Update the **ctr_client_id** and **ctr_client_password** variables with  CTR client ID and Client Password you got in step 2. **( Or you can use the GUI in the lab simulator )** 
5. In the **config.py** script, depending on your region, uncomment the related **host=xxx** and **host_for_token=xxx** variables. **Notice** DCLOUD demos are located in the US. **( Or you can use the GUI in the lab simulator )**
